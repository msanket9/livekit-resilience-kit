...
I've lived that gap. Running an IoT platform with 80+ ESP32/Modbus field devices on
cellular backhaul meant constantly debugging cellular gateways going randomly offline. I
built a synthetic evaluation harness that cut incident-detection time from 4+ hours to
under 5 minutes. This project applies the same instinct, testing the real failure mode
instead of just the happy path, to LiveKit's real-time transport layer, using
